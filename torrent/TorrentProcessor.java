...
public class TorrentProcessor {
    ...
    public TorrentFile getTorrentFile(Map m) {
        ...
        if (m.containsKey("authHash")) // optional key
            this.torrent.authHash = new String((byte[]) m.get("authHash"));
        ...
        if (m.containsKey("info")) {
            ...
            if (info.containsKey("files")) {
                ...
                for (int i = 0; i < multFiles.size(); i++) {
                    ...
                    this.torrent.total_length += ((Long) ((Map) multFiles.get(i)).get("length")).intValue();
    ...
    /**
     * Sets the authentication hash of the torrent
     * 
     * @param aHash String
     */
    public void setAuthHash(String aHash) {
        this.torrent.authHash = aHash;
    }
    ...
    /**
     * Sets the directory the files have to be saved in (in case of multiple files
     * torrent)
     * 
     * @param name String
     */
     ...
     * 
     * @param l A list containing the File or String object representing the files
     *          to be added
    ...
    public byte[] generateTorrent(TorrentFile torr) {
        ...
        if (torr.authHash.length() > 0)
            map.put("authHash", torr.authHash);
